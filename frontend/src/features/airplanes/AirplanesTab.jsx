...
const AirplaneForm = ({ initialData, onSubmit, onCancel }) => {
    ...
    const [planeData, setPlaneData] = useState(isEditMode ? initialData : { name: 'Máy bay M', economySeats: 12, businessSeats: 6 });
    ...
    const handleSubmit = () => { onSubmit({ ...planeData, totalSeats }); }
        ...
        <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 p-4 border rounded-lg bg-white">
                <div className="grid grid-cols-6 gap-2">
                    {Array.from({ length: planeData.businessSeats }, (_, i) => (<div key={`b-${i}`} className="w-10 h-10 bg-teal-200 rounded flex items-center justify-center text-teal-800 text-xs font-semibold">{`B${i + 1}`}</div>))}
                    {Array.from({ length: planeData.economySeats }, (_, i) => (<div key={`e-${i}`} className="w-10 h-10 bg-cyan-200 rounded flex items-center justify-center text-cyan-800 text-xs">{`E${i + 1}`}</div>))}
                </div>
            </div>
        </div>
    );
...
const AirplanesList = ({ airplanes, onEdit, onCreate, onDelete }) => {
    ...
    const filteredAirplanes = airplanes.filter(plane => plane.name.toLowerCase().includes(searchTerm.toLowerCase()) || plane.id.toLowerCase().includes(searchTerm.toLowerCase()) );
        ...
        <div className="p-6">
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left">
                    <thead className="bg-gray-50 border-b"><tr><th className="p-4 font-semibold text-gray-600">Mã máy bay</th><th className="p-4 font-semibold text-gray-600">Tên máy bay</th><th className="p-4 font-semibold text-gray-600">Số lượng ghế</th><th className="p-4 font-semibold text-gray-600 text-center">Thao tác</th></tr></thead>
                    <tbody>
                        {filteredAirplanes.map(plane => (
                            <tr key={plane.id} className="border-b hover:bg-gray-50 transition-colors">
                                <td className="p-4 font-mono text-blue-600">{plane.id}</td><td className="p-4 text-gray-800">{plane.name}</td><td className="p-4 text-gray-800">{plane.totalSeats}</td>
...
const AirplanesTab = ({ airplanes, onCreate, onEdit, onDelete }) => {
    ...
    const confirmDelete = () => { onDelete(airplaneToDelete); setAirplaneToDelete(null); }
    const cancelDelete = () => setAirplaneToDelete(null);
    const handleCancel = () => { setEditingAirplane(null); setSubTab('list'); }
    const handleSave = (planeData) => { if (editingAirplane) { onEdit({ ...editingAirplane, ...planeData }); } else { onCreate(planeData); } setSubTab('list'); setEditingAirplane(null); }
    ...
    return (
        <div>
            <div className="px-6 pt-4 pb-2 border-b flex items-center justify-between"><div className="flex items-center space-x-2"><SubTabButton value="list">Danh sách máy bay</SubTabButton><SubTabButton value="create">Tạo máy bay mới</SubTabButton>{subTab === 'edit' && (<span className="px-6 py-2 rounded-full text-sm font-semibold bg-blue-600 text-white shadow animate-fade-in">Chi tiết máy bay</span>)}</div></div>
