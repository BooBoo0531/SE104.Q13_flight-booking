...
// 0. Confirmation Modal Component
const ConfirmationModal = ({ message, onConfirm, onCancel }) => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 animate-fade-in">
        <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-sm">
            <p className="text-gray-800 mb-6 text-center">{message}</p>
            <div className="flex justify-end gap-4">
                <button onClick={onCancel} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 transition-colors">Hủy</button>
                <button onClick={onConfirm} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">Xác nhận Xóa</button>
            </div>
        </div>
    </div>
);
...
const AuthScreen = ({ onLogin }) => {
    ...
    const TabButton = ({ tabName, children }) => (<button onClick={() => setActiveTab(tabName)} className={`w-full py-3 text-sm font-semibold rounded-t-lg transition-colors focus:outline-none ${ activeTab === tabName ? 'bg-white text-blue-600' : 'bg-gray-100 text-gray-500 hover:bg-gray-200' }`}>{children}</button>);
    const FormInput = (props) => (<input className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-shadow" {...props}/>);
    return (<div className="w-full max-w-md mx-auto animate-fade-in"><div className="text-center mb-8"><PlaneTakeoff className="mx-auto h-12 w-12 text-blue-500" /><h1 className="text-3xl font-bold text-gray-800 mt-4">Hệ Thống Quản Lý Bay</h1><p className="text-gray-500 mt-2">Vui lòng đăng nhập để tiếp tục</p></div><div className="bg-white rounded-xl shadow-lg overflow-hidden"><div className="flex"><TabButton tabName="login">Đăng nhập</TabButton><TabButton tabName="register">Đăng ký</TabButton></div><div className="p-8">{activeTab === 'login' && (<div className="space-y-4"><FormInput type="email" placeholder="Email" defaultValue="admin@example.com" /><FormInput type="password" placeholder="Mật khẩu" defaultValue="password" /><a href="#" className="block text-right text-sm text-blue-500 hover:underline">Quên mật khẩu?</a><button onClick={onLogin} className="w-full bg-blue-500 text-white font-bold py-3 rounded-lg hover:bg-blue-600 transition-all transform hover:scale-105 shadow-md hover:shadow-lg">Đăng nhập</button></div>)}{activeTab === 'register' && (<div className="space-y-4"><FormInput type="text" placeholder="Họ và tên" /><FormInput type="email" placeholder="Email" /><FormInput type="password" placeholder="Mật khẩu" /><button onClick={onLogin} className="w-full bg-green-500 text-white font-bold py-3 rounded-lg hover:bg-green-600 transition-all transform hover:scale-105 shadow-md hover:shadow-lg">Tạo tài khoản</button></div>)}</div></div></div>);
};
...
// 2q. The main screen containing everything
const MainScreen = () => {
